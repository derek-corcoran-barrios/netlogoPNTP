...
to load-gis
  set pathation gis:load-dataset "senderos_90.asc"
...
to go
 ...
 ask node 100 [ show nw:path-to node 200  ]
 ask node 100 [ show nw:turtles-on-path-to node 200 ] 
 tick
end
...
to move
  ...
  [ face destination
    ...
    set age (age + 1 )
    while [pcolor != red] [back 1 left 5 forward 1]
  ]
